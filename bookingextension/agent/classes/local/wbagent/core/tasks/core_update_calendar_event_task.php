...
class core_update_calendar_event_task extends core_task_base implements task_trigger_provider_interface {
    ...
    public function execute(array $input, int $cmid, int $userid): array {
        ...
        if ($eventid <= 0) { return ['status' => 'error', 'detail' => $this->localized_string('agent_booking_core_eventid_required', null, $lang), 'resultid' => null]; }
        ...
        if (!$event) { return ['status' => 'error', 'detail' => $this->localized_string('agent_booking_core_calendar_event_not_found', null, $lang), 'resultid' => null]; }
        ...
        if (array_key_exists('title', $input)) { $payload->name = trim((string)$input['title']); }
        if (array_key_exists('description', $input)) { $payload->description = trim((string)$input['description']); $payload->format = FORMAT_HTML; }
