...
class core_create_group_task extends core_task_base implements task_trigger_provider_interface {
    ...
    public function validate(array $input, int $cmid): array {
        ...
        if (trim((string)($input['name'] ?? '')) === '') { $errors[] = get_string('agent_booking_core_group_name_required', 'mod_booking'); }
        if (empty($input['confirmed'])) {
            $issues[] = ['code' => 'CONFIRMATION_REQUIRED', 'severity' => 'needs_confirmation', 'user_question' => get_string('agent_booking_core_confirm_create_group', 'mod_booking'), 'remedy_options' => ['CONFIRM', 'CANCEL']];
        }
