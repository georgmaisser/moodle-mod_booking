...
use bookingextension_agent\local\wbagent\booking\support\booking_mutation_validation;
...
class booking_task_mutation_execute_service {
    ...
    public function execute(string $taskname, array $input, int $cmid, int $userid, booking_task_support $support): ?array {
        ...
        if (!$cm) {
            return [
                'status' => 'error',
                'detail' => get_string('agent_booking_invalid_course_module', 'mod_booking'),
                'resultid' => null,
            ];
        }
        ...
        if (!empty($input['enrolledincoursequery'])) {
            ...
            if (empty($restrictioncourses['courseids'])) {
                return [
                    'status' => 'error',
                    'detail' => get_string('agent_booking_no_valid_course_enrolled', 'mod_booking'),
                    'resultid' => null,
                ];
            }
        ...
        if ($taskname === create_option_task::TASK_NAME) {
            ...
            if (empty($optionids)) {
                return [
                    'status' => 'error',
                    'detail' => get_string('agent_booking_no_matching_options_to_update', 'mod_booking'),
                    'resultid' => null,
                ];
            }
