...
/**
 * External service: store a purchased Wunderbyte API key on the provider.
 *
...
/**
 * Store a customer-supplied (purchased) LiteLLM key on the aiprovider_wunderbyte instance.
 *
 * The key is quick-checked and then written to the provider config against the hard-coded Wunderbyte
 * LLM endpoint (same store path as the trial). The key is never sent to the LLM as content; it is a
 * config write, gated on the same onboarding capability as the trial.
 *
 * @package    bookingextension_agent
 * @copyright  2026 Wunderbyte GmbH <info@wunderbyte.at>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class store_provider_apikey extends external_api {
    /**
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'apikey' => new external_value(PARAM_RAW, 'The purchased Wunderbyte API key.'),
        ]);
    }

    /**
     * Store the supplied API key on the Wunderbyte provider instance.
     *
     * @param int $contextid
     * @param string $apikey
     * @return array
     */
    public static function execute(int $contextid, string $apikey): array {
        global $USER;
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            'success' => new external_value(PARAM_BOOL, 'Whether the key was stored.'),
