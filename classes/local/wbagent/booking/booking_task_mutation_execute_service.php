...
use mod_booking\local\wbagent\options\tasks\bulk_update_options_task;
use mod_booking\local\wbagent\options\tasks\create_option_task;
use mod_booking\local\wbagent\options\tasks\update_option_task;
...
class booking_task_mutation_execute_service {
    ...
    public function preflight_validate(string $taskname, array $input, int $cmid, int $userid): array {
        ...
        if ($taskname === bulk_update_options_task::TASK_NAME) {
            ...
            if (empty($optionids)) {
                $errors[] = get_string('agent_booking_no_matching_options_to_update', 'bookingextension_agent');
            }
        }
