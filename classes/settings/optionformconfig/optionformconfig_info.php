...
class optionformconfig_info {
    ...
    public static function render_list_of_roles() {

        global $OUTPUT;

        $data['roles'] = self::get_list_of_configurable_roles();

        echo $OUTPUT->render_from_template('mod_booking/settings/optionformconfig/roleslist', $data);

    }

    /**
     * Just fetches all theroles we need.
     * We might need to restrict this in the future.
     * @param int $contextlevel 0 is no restriction.
     * @return array
     * @throws dml_exception
     */
    private static function get_list_of_configurable_roles(int $contextlevel = 0) {

        global $DB;

        $sql = "SELECT r.id, COALESCE(r.name, r.shortname) as name, r.shortname
                FROM {role} r
                JOIN {role_context_level} rcl ON r.id = rcl.roleid
        ";

        $roles = $DB->get_records_sql($sql);

        $data = array_map(fn($a) => (array)$a, $roles);
